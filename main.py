...
class Main:
    ...
    def __init__(self):
        ...
        self.main_obj.delete6.clicked.connect(self.delete_video)
        self.main_obj.video6.clicked.connect(self.play_videoFile)
    ...
    # Here we show the report of the selected person
    def show_report(self):
        if os.path.exists("Report/report.csv"):
            df = pd.read_csv("Report/report.csv")
            row = df[df['ID'] == int(self.Id)]

            file_data = list(row.iloc[:].values)
    ...
    def Start_training_thread(self):
        ...
        if self.training_thread.isAlive():
            return
    ...
    def delete_video(self):
        ...
        video_path = row[4].text()
        df = pd.read_csv("Report/report.csv")
        df = df.drop(df[df['VIDEO'] == video_path].index)
        df.to_csv("Report/report.csv", index=False, sep=',')
    ...
    def update_Graph(self, path):
        ...
        self.main_obj.progressBar4.setValue(
            int((self.trainer_model.get_epoch() / int(self.main_obj.epochs.text())) * 100))
    ...
    def show_ipdialogue(self):
        ...
        if self.Id != "" and self.name != "":
            ...
            msg.setText("Select a sample to track")
    ...
    def Track_Subject_Over_Ip_Camera(self):
        ...
        if not self.validate_ip(self.ipaddress):
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Ipaddress is not valid")
            msg.setWindowTitle("Warning")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
        ...
        try:
            ...
            # Save the video of that subject in his folder
            if not os.path.exists("Report/" + self.Id):
                os.mkdir("Report/" + self.Id)

            n = len(os.listdir("Report/" + self.Id))

            # Create the video recording object
            video_path = 'Report/' + str(self.Id) + '/' + self.name + "." + self.Id + "." + str(n) + ".mp4"
            vid_out = cv2.VideoWriter(video_path, fourcc, 30.0, (int(w), int(h)))
                ...
                if len(faces) is not 0:
                    ...
                    for face in faces:
                        ...
                        if int(self.Id) == df['ID'][Id - 1]:
                            ...
                            cv2.putText(im, "confidence:" + str(int(conf)) + "%", (x1, y2 + 30), font, 1, (0, 255, 0),
                                        1)
                            ts = time.time()
                            date = datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                            timeStamp = datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                            report_df.loc[len(report_df)] = [self.Id, self.name, date, timeStamp, video_path]
    ...
    def Track_subject_on_WebCam(self):
        ...
        if self.Id == "" and self.name == "":
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Please select a subject to track")
            msg.setWindowTitle("Warning")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
        ...
        try:
            ...
            msg.setIcon(QMessageBox.Warning)
            msg.setText(e)
            msg.setWindowTitle("Web camera not found error")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
            ...
            if len(faces) != 0:
                ...
                for face in faces:
                    ...
                    if int(self.Id) == df['ID'][Id - 1]:
                        ...
                        # Now save the track frame date time and name id in the dataframe
                        ts = time.time()
                        date = datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                        timeStamp = datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                        report_df.loc[len(report_df)] = [self.Id, self.name, date, timeStamp, video_path]
                    ...
                    # Save the last record of the dataframe
                    report_df = report_df.drop_duplicates(subset=['ID'], keep='first')
                    report_df.to_csv("Report/report.csv", index=False)
    ...
    def Track_subject_on_video(self):
        ...
        path = QFileDialog.getOpenFileName(caption="Select the video to track the subject", directory="",
                                           filter="mp4 videos (*.mp4);;webm videos (*.webm);;avi videos (*.avi);;All "
                                                  "videos(*.*)",
                                           options=options)
        recognizer = My_model.AD_Model()
        ...
        if path[0] is "":
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("No video file selected")
            msg.setWindowTitle("Warning")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
        ...
        # Create the video recording object to start the object
        video_path = 'Report/' + str(self.Id) + '/' + self.name + "." + self.Id + "." + str(n) + ".mp4"
        vid_out = cv2.VideoWriter(video_path, fourcc, 30.0, (int(w), int(h)))
                ...
                if ret:
                    ...
                    if len(faces) is not 0:
                        ...
                        for face in faces:
                            ...
                            # Keep only the first record to save in the report file
                            report_df = report_df.drop_duplicates(subset=['ID'], keep='first')
                            report_df.to_csv("Report/report.csv", index=False)
                    else:
                        msg = " Searching for face"
        ...
        for category in categories:
            ...
            for img_name in img_names:
                ...
                try:
                    ...
                    data.append(resized)
                    target.append(img_path.split('.')[1])
    ...
    def StartTraining(self):
        ...
        # Now make the thread close
        self.training_thread.thread_active = False
    ...
    def sample_from_camera(self):
        ...
        if not self.check_name_exists(self.name):
            ...
            if self.age.isdigit() and self.phone.isdigit() and self.address.isalpha():
                ...
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("invalid Input values")
                msg.setWindowTitle("Warning")
                msg.setStandardButtons(QMessageBox.Ok)
                msg.exec_()
        else:
    ...
    def sample_from_gallery(self):
        ...
        if len(images[0]) == 0:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("No Images are Provided")
            msg.setWindowTitle("Warning")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
    ...
    def insert(self):
        ...
        if "" in self.row:
            ...
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Sample are not taken")
            msg.setWindowTitle("Warning")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
            return
            ...
            # Set the flag of sample taken to False
            self.sample_taken = False
